...
struct Node
{
    int y, x;           // Pozycje węzła na mapie
    int g_cost, h_cost; // Koszt dotarcia i heurystyka
    Node *parent;       // Wskaźnik do węzła rodzica
...
// Funkcja Heurystyki (Odległość Euklidesowa)
int heuristic(int y1, int x1, int y2, int x2)
{
    return static_cast<int>(sqrt((y2 - y1) * (y2 - y1) + (x2 - x1) * (x2 - x1)));
}

// Sprawdzenie, czy punkt jest w granicach mapy i nie jest przeszkodą
...
void AStarSearch(vector<vector<int>> &grid, Node start, Node goal)
{
    ...
    while (!openSet.empty())
    {
        ...
        if (*current == goal)
        {
            ...
            // Zwolnienie pamięci zajętej przez węzły
            while (!openSet.empty())
            {
                delete openSet.top();
                openSet.pop();
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            {
                int newGCost = current->g_cost + 1; // Koszt ruchu do sąsiada
